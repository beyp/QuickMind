...
class PromptBar(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # Bouton Kanban
        self._kanban_btn = ctk.CTkButton(self, text="📊 Kanban",
            width=100, height=36,
                ...
                for f in files:
                    try:
                        ...
                        if Path(path.strip()).exists():
                            ...
                            self.after(0, lambda: self._analyze_btn.configure(fg_color="#7B2FBE"))
                            self.after(1000, lambda: self._analyze_btn.configure(fg_color="#4a2a6a"))
                            break
    ...
    def _open_file_picker(self):
        ...
        if path: self._open_analysis(path)
