...
class TaskForm(ctk.CTkToplevel):
    ...
    def __init__(self, master, task, category_id, on_save):
        ...
        self._task_id_tmp = task.id if task else None  # pour sauvegarder les images
    ...
    def _on_ctrl_v(self, event):
        """Intercepte Ctrl+V — colle le texte normal OU une image."""
        try:
            from core.clipboard_image import has_image_in_clipboard
            if has_image_in_clipboard():
                # Image dans presse-papier → coller comme PJ
                self._paste_image()
                return "break"   # empeche le paste texte normal
            # Pas d image → laisser le comportement normal (paste texte)
            return None
        except Exception:
            return None

    def _paste_image(self):
        """Colle l image du presse-papier comme pièce jointe."""
        try:
            from core.clipboard_image import save_clipboard_image, get_thumbnail

            # Pour une nouvelle tache, on utilise un ID temporaire
            tmp_id = self._task_id_tmp or 0
            result = save_clipboard_image(tmp_id)

            if not result:
                self._clip_label.configure(
                    text="Aucune image dans le presse-papier",
                    text_color="gray"
                )
                return
            ...
            # Stocker le chemin pour la sauvegarde
    ...
    def _show_thumbnail(self, thumb_path: str, img_path: str):
        """Affiche la miniature de l image collee."""
        try:
            ...
            # Garder la reference pour eviter GC
            ...
            # Afficher miniature avec tkinter Label natif
            import tkinter as tk
            lbl = tk.Label(thumb_frame, image=thumb,
                          bg="#2b2b2b", cursor="hand2")
            lbl.pack(padx=4, pady=4)
            lbl.bind("<Button-1>", lambda e, p=img_path: __import__("os").startfile(p))

            # Bouton supprimer la miniature
            ctk.CTkButton(thumb_frame, text="✕", width=20, height=20,
            ...
            print(f"[Clipboard] Miniature non affichee : {e}")
    ...
    def _browse(self):
        ...
        if path:
            ...
            self._att_label.configure(
                text=f"PJ : {os.path.basename(path)}",
                text_color="#88BBFF")
